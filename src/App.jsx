...
function drawHand(ctx, landmarks, w, h) {
  ...
  pts.forEach((pt, i) => {
    ...
    ctx.shadowColor = isKnuckle ? "#ff00aa" : "#00ffcc";
    ctx.shadowBlur = 18;
    ctx.beginPath();
    ctx.arc(pt.x, pt.y, isKnuckle ? 6 : 4, 0, Math.PI * 2);
    ctx.fillStyle = isKnuckle ? "#ff00aa" : "#00ffcc";
    ctx.fill();
  });
}
...
function classifyGesture(lm) {
  ...
  if (index && !middle && !ring && !pinky && !thumbUp && !thumbOut)
    return "point";
...
const GESTURE_CONFIG = {
  ...
  three: {
    ...
    action: "timer",
    color: "#44ccff",
    description: "3 sec timer started!",
  },
  four: {
    emoji: "4️⃣",
    label: "Four",
...
const GESTURE_SOUNDS = {
  ...
  rock_on: () => {
    ...
    setTimeout(() => playTone(165, "sawtooth", 0.15), 100);
    setTimeout(() => playTone(220, "sawtooth", 0.3), 200);
  },
  three: () => {
    [523, 659, 784].forEach((f, i) =>
      setTimeout(() => playTone(f, "sine", 0.15, 0.2), i * 120),
    );
  },
  four: () => playTone(587, "sine", 0.2, 0.2),
  call_me: () => {
    [700, 600, 700].forEach((f, i) =>
      setTimeout(() => playTone(f, "sine", 0.18, 0.15), i * 200),
    );
  },
  ok: () => {
    playTone(523, "sine", 0.08);
    setTimeout(() => playTone(659, "sine", 0.12), 80);
    setTimeout(() => playTone(1047, "sine", 0.2), 160);
  },
  vulcan: () => {
    [400, 500, 600, 500, 400].forEach((f, i) =>
      setTimeout(() => playTone(f, "sine", 0.12, 0.1), i * 80),
    );
  },
  finger_gun: () => {
    playTone(800, "square", 0.05, 0.3);
    setTimeout(() => playTone(200, "sawtooth", 0.3, 0.2), 50);
  },
};
...
function createParticles(count = 80) {
  return Array.from({ length: count }, (_, i) => ({
    ...
    vy: 2 + Math.random() * 3,
    color: ["#ff00aa", "#00ffcc", "#ffcc00", "#ff8800", "#8888ff", "#44ccff"][
...
export default function EmojiMirror() {
  ...
  const {
    handLandmarkerRef,
    loadModel,
    detectFromImage: hookDetectFromImage,
  ...
  const loadModelWrapper = useCallback(
    async (mode) => {
  ...
  const renderGrid = useCallback(() => {
    ...
    if (drawPathRef.current.length > 1) {
      ...
      ctx.shadowBlur = 8;
      ctx.beginPath();
      ctx.moveTo(drawPathRef.current[0].x, drawPathRef.current[0].y);
      drawPathRef.current.forEach((p) => ctx.lineTo(p.x, p.y));
      ctx.stroke();
      ctx.shadowBlur = 0;
    }
  }, [T.grid]);
  ...
  const checkCombo = useCallback(
    (gesture) => {
      ...
      if (COMBOS[key]) {
        ...
        setComboLog((prev) => [
          { combo: COMBOS[key], time: new Date().toLocaleTimeString() },
          ...prev.slice(0, 9),
        ]);
        comboRef.current = [];
      }
  ...
  const triggerAction = useCallback(
    (gesture, lm) => {
      ...
      const effectiveAction =
        gestureMap[gesture] || GESTURE_CONFIG[gesture]?.action;
      const cfg = GESTURE_CONFIG[gesture];
      setLastAction({ gesture, cfg, time: Date.now() });
      setLog((prev) => [
        { gesture, cfg, time: new Date().toLocaleTimeString() },
        ...prev.slice(0, 49),
      ]);
      ...
      if (effectiveAction === "save_note")
        setNotes((prev) => [
          {
            text: `Note saved at ${new Date().toLocaleTimeString()}`,
            time: Date.now(),
          },
          ...prev,
        ]);
      if (effectiveAction === "delete") setNotes((prev) => prev.slice(1));
      if (effectiveAction === "draw") {
        setDrawMode((m) => !m);
        drawingRef.current = !drawingRef.current;
        showNotif(
          drawingRef.current ? "✏️ Draw mode ON" : "✏️ Draw mode OFF",
          "#8888ff",
        );
      }
      ...
      if (effectiveAction === "notify")
        showNotif("🔔 Notification sent!", "#44ffaa");
      ...
      if (effectiveAction === "screenshot") {
        ...
        if (canvas) {
          ...
          a.href = dataUrl;
          a.download = `emoji-mirror-${Date.now()}.png`;
          a.click();
        }
      }
  ...
  const updateDrawing = useCallback((lm, w, h) => {
    ...
    if (drawPathRef.current.length > 200)
      drawPathRef.current = drawPathRef.current.slice(-200);
  }, []);
    ...
    (videoEl, mirror = false) => {
      const onLandmarks = (handsData, canvas) => {
        ...
        if (handsData) {
          ...
          const landmarks = handsData.hands
            ? handsData.hands[0]?.landmarks
            : handsData;
      ...
      loopRef.current = hookStartDetectionLoop(
        videoEl,
        canvasRef,
        onLandmarks,
        mirror,
  ...
  useEffect(() => {
    ...
    const tick = () => {
      local = local
        ...
        .filter((p) => p.y < 120);
      setParticles([...local]);
      if (local.length > 0) frame = requestAnimationFrame(tick);
    };
  ...
  const topGesture = Object.entries(gestureCount).sort(
    (a, b) => b[1] - a[1],
  )[0];
    ...
    >
      ...
      {notification && (
        <div
          style={{
            ...
            right: 20,
            padding: "12px 18px",
            background: notification.color + "30",
      ...
      <div style={{ padding: 20, maxWidth: 1280, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <h1 style={{ marginBottom: 2, fontSize: 28, letterSpacing: 3 }}>
            👁️ EMOJI MIRROR
          </h1>
          <p
            style={{ fontSize: 10, color: T.dim, letterSpacing: 2, margin: 0 }}
          >
            Gesture Recognition • Hand Tracking • 3D Physics Engine
          </p>
        </div>
        ...
        >
          ...
          >
            {inputMode === "webcam" ? "LIVE" : "STATIC"} • {fps} FPS •{" "}
            {fmtTime(sessionTime)}
          </div>
        </div>
        ...
        >
          {[
            ...
            <button
              ...
              style={{
                ...
                padding: "10px",
                background:
                  inputMode === m.key ? `${T.accent}22` : "transparent",
                border: `1px solid ${inputMode === m.key ? T.accent : T.border}`,
                color: inputMode === m.key ? T.accent : T.dim,
                cursor: "pointer",
                borderRadius: 3,
                fontSize: 11,
                letterSpacing: 2,
          ...
          <button
            ...
            style={{
              flex: 1,
              padding: "10px",
              background: show3D ? `${T.accent}22` : "transparent",
              border: `1px solid ${show3D ? T.accent : T.border}`,
              color: show3D ? T.accent : T.dim,
              cursor: "pointer",
              borderRadius: 3,
              fontSize: 11,
              letterSpacing: 2,
        ...
        {inputMode === "image" && (
          <button
            ...
            style={{
              width: "100%",
              padding: "12px",
              background: scanning ? "#444" : "#00ffcc33",
        ...
        {inputMode === "video" && (
          <button
            ...
            style={{
              ...
              padding: "12px",
              marginBottom: 20,
              background: videoPlaying ? "rgba(255,68,68,0.2)" : "#44ffaa33",
              border: "1px solid #44ffaa66",
              color: "#44ffaa",
              cursor: videoPlaying ? "not-allowed" : "pointer",
              borderRadius: 3,
              fontSize: 11,
              letterSpacing: 2,
            }}
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
            <button
              ...
              style={{
                flex: 1,
                padding: "12px",
                background: webcamActive ? "rgba(255,68,68,0.2)" : "#ff66ff33",
        ...
        >
          <div style={{ marginBottom: 8, letterSpacing: 2, color: T.dim }}>
            RECENT
          </div>
          {log.slice(0, 10).map((entry, i) => (
            ...
            >
              <span>
                {entry.cfg?.emoji} {entry.cfg?.label}
              </span>
