/**
 * ThreeCanvas.jsx — Emoji Mirror 3D Layer
 * Loads: public/models/trainengine.glb
 * Place in: src/ThreeCanvas.jsx
 */
...
// ─── CONFIG ──────────────────────────────────────────────────────────────────
const WORLD_SCALE  = 6;      // how wide the hand movement maps to 3D space
const DEPTH_SCALE  = 3;      // z depth sensitivity
const LERP_ALPHA   = 0.1;    // smoothness (lower = smoother)
const TRAIL_MAX    = 100;    // max paint dots
const TRAIL_RADIUS = 0.09;
const CAMERA_Z     = 18;
const MODEL_SCALE  = 0.02;   // start small, adjust if model is too big/small
...
export default function ThreeCanvas({ landmarksRef, gesture }) {
  ...
  useEffect(() => {
    ...
    const fallback = new THREE.Mesh(
      ...
      new THREE.MeshPhongMaterial({ color:0x00ffcc, emissive:new THREE.Color(0x00ffcc).multiplyScalar(0.1), shininess:120, wireframe:false, transparent:true, opacity:0.85 })
    );
    const fallbackWire = new THREE.Mesh(
      new THREE.DodecahedronGeometry(1.2, 0),
      new THREE.MeshBasicMaterial({ color:0xff00aa, wireframe:true, transparent:true, opacity:0.2 })
    );
    fallback.add(fallbackWire);
    scene.add(fallback);

    // Save refs before model loads
    stateRef.current = { renderer, scene, camera, cursor, trailGroup, pLight1, pLight2, stars, grid, hero: fallback };
    ...
    loader.load(
      ...
      (err) => {
        ...
        // fallback dodecahedron stays in scene
      }
    );
    ...
    const animate = () => {
      ...
      const { renderer, scene, camera, cursor, trailGroup, pLight1, pLight2, stars, hero } = stateRef.current;
      ...
      if (lm && lm.length >= 21) {
        ...
        if (gesture === "ok" || pinch < 0.08) {
          targetScale.current = THREE.MathUtils.clamp(
            THREE.MathUtils.mapLinear(pinch, 0.02, 0.15, 0.3, 2.8), 0.2, 3.0
          );
        } else {
        ...
        if (gesture === "thumbs_down") {
          trailRef.current.forEach(dot => {
            trailGroup.remove(dot); dot.geometry.dispose(); dot.material.dispose();
          });
          trailRef.current = [];
        }
      ...
      if (hero) {
        if (gesture === "fist") {
          ...
          // Loosely follow hand
          hero.position.x = THREE.MathUtils.lerp(hero.position.x, currentPos.current.x * 0.45, 0.05);
          hero.position.y = THREE.MathUtils.lerp(hero.position.y, currentPos.current.y * 0.45, 0.05);
          hero.position.z = THREE.MathUtils.lerp(hero.position.z, currentPos.current.z * 0.3,  0.05);
        ...
        hero.traverse(child => {
          if (child.isMesh && child.material && child.material.emissive) {
            ...
            const target = (gesture && gesture !== "none" && gesture !== "unknown") ? pulse * 0.35 : 0.05;
            child.material.emissiveIntensity = THREE.MathUtils.lerp(child.material.emissiveIntensity, target, 0.08);
          }
        });
      }
  ...
  return (
    <div style={{ position:"relative", width:"100%", height:"100%" }}>
      ...
      <div style={{ position:"absolute", bottom:10, left:10, pointerEvents:"none",
                    fontSize:9, letterSpacing:2, color:"rgba(0,255,204,0.5)",
                    fontFamily:"'Courier New',monospace", textTransform:"uppercase" }}>
        THREE.JS · {gestureLabels[gesture] || (gesture ? gesture.replace(/_/g," ") : "waiting…")}
      </div>

      {/* Model name top-left */}
      <div style={{ position:"absolute", top:10, left:10, pointerEvents:"none",
                    fontSize:8, letterSpacing:1, color: modelLoaded ? "rgba(0,255,204,0.4)" : "rgba(255,136,0,0.6)",
      ...
      <div style={{ position:"absolute", top:10, right:10, pointerEvents:"none",
                    fontSize:8, letterSpacing:1, color:"rgba(0,255,204,0.3)",
                    fontFamily:"'Courier New',monospace" }}>
        SCALE {currentScale.current.toFixed ? currentScale.current.toFixed(2) : "1.00"}
      </div>
    </div>
  );
}

// expose cursorRing for animation — grab from cursor children
const cursorRing = { rotation: { z: 0 } };
